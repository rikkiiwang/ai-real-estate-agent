...
| Voice (engagement pillar¹) | Omnichannel: one thread across Voice/SMS/Email/Chat (R4) | ✅ reachable — one **persisted `Conversation`** thread every channel reads/writes (the sidebar shows the cross-channel history; the brain is keyed to it). **Real `TwilioSms` / `SendgridEmail` adapters** send when keys are set (else `Simulated` — demo stays external-call-free). **Inbound webhooks** (`/inbound/sms` `/inbound/email` `/inbound/voice`, token-guarded) append to the same thread, orchestrate, and reply; the Go voice service relays into it. Handoffs carry the full cross-channel transcript. Only the live carrier credentials are deferred |
...
Python brain **241** passing, Rails **323** passing. See `docs/ARCHITECTURE.md`
